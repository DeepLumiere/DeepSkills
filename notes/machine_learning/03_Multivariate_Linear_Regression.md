# Complete Machine Learning Notes: Multivariate Linear Regression
...
> **Files Integrated:** Multivariate Linear Regression-Shared.pdf (41 Slides)
...
### 12.2 Master Solved Problem: Direct Matrix Inversion and Normal Equations

> [!IMPORTANT]
> **Complete End-to-End Problem (No Hand-Waving or Guessing):**
> Consider a dataset with $m = 4$ observations and $n = 2$ features ($x_1$: Engine Size, $x_2$: Cylinders) predicting Target $y$ (Fuel Consumption):
>
> | Sample ($i$) | Bias ($x_0$) | $x_1$ | $x_2$ | Target $y$ |
> | :---: | :---: | :---: | :---: | :---: |
> | **1** | 1 | 1 | 0 | **3** |
> | **2** | 1 | 0 | 1 | **4** |
> | **3** | 1 | 1 | 1 | **6** |
...
>
> **Task:**
> 1. Formulate Design Matrix $X$ and Target Vector $Y$.
...
#### Step 3: Compute Determinant and Invert $(X^T X)^{-1}$
For a $3 \times 3$ matrix $M$:
$$
\det(X^T X) = 4(6 \times 3 - 3 \times 3) - 4(4 \times 3 - 3 \times 3) + 3(4 \times 3 - 6 \times 3)
$$
$$
\det(X^T X) = 4(18 - 9) - 4(12 - 9) + 3(12 - 18) = 4(9) - 4(3) + 3(-6) = 36 - 12 - 18 = \mathbf{6} \neq 0
$$

Since $\det(X^T X) = 6 \neq 0$, the matrix is strictly non-singular and invertible!
Computing the Cofactor Matrix $C$:
- $C_{11} = +(18 - 9) = 9$
- $C_{12} = -(12 - 9) = -3$
- $C_{13} = +(12 - 18) = -6$
- $C_{21} = -(12 - 9) = -3$
...
Taking the transpose $\text{Adj}(X^T X) = C^T$:
$$
...
#### Step 5: Solve $\theta^* = (X^T X)^{-1} X^T Y$

$$
\theta^* = \frac{1}{6} \begin{bmatrix}
9 & -3 & -6 \\
-3 & 3 & 0 \\
-6 & 0 & 8
\end{bmatrix}
...
- $\theta_0^* = \frac{1}{6} [9(21) - 3(25) - 6(18)] = \frac{1}{6} [189 - 75 - 108] = \frac{6}{6} = \mathbf{1.0}$
- $\theta_1^* = \frac{1}{6} [-3(21) + 3(25) + 0(18)] = \frac{1}{6} [-63 + 75 + 0] = \frac{12}{6} = \mathbf{2.0}$
- $\theta_2^* = \frac{1}{6} [-6(21) + 0(25) + 8(18)] = \frac{1}{6} [-126 + 0 + 144] = \frac{18}{6} = \mathbf{3.0}$

Optimal Parameter Vector:
$$
\theta^* = \begin{bmatrix} 1.0 \\ 2.0 \\ 3.0 \end{bmatrix} \implies \hat{y} = 1.0 + 2.0 x_1 + 3.0 x_2
$$

#### Step 6: Prediction Verification, Residual Table & Goodness of Fit

| Sample ($i$) | Actual $y^{(i)}$ | Prediction $\hat{y}^{(i)} = 1 + 2x_1 + 3x_2$ | Residual $e^{(i)} = y^{(i)} - \hat{y}^{(i)}$ | Squared Error $(e^{(i)})^2$ |
| :---: | :---: | :---: | :---: | :---: |
| **1** | 3 | $1 + 2(1) + 3(0) = 3$ | $3 - 3 = 0$ | $0.0$ |
| **2** | 4 | $1 + 2(0) + 3(1) = 4$ | $4 - 4 = 0$ | $0.0$ |
| **3** | 6 | $1 + 2(1) + 3(1) = 6$ | $6 - 6 = 0$ | $0.0$ |
| **4** | 8 | $1 + 2(2) + 3(1) = 8$ | $8 - 8 = 0$ | $0.0$ |
| **Total** | $\sum y = 21$ | $\sum \hat{y} = 21$ | $\sum e = 0$ | $\text{SSE} = \mathbf{0.0}$ |

- **Mean Squared Error (MSE):** $\frac{1}{4} \sum (e^{(i)})^2 = \mathbf{0.0}$
- **Coefficient of Determination ($R^2$):** $\mathbf{1.0}$ (100% of variance explained by model).
