# Chapter 1: Full Stack Development Basics & REST Architecture
...
## 1. Chapter Overview
This chapter provides the structural, architectural, and protocol foundation for web application engineering. It covers:
...
- Popular Web Development Stacks (LAMP, MEAN, MERN, Ruby on Rails, Django, Spring Boot, Serverless, Flutter/React Native).
- REpresentational State Transfer (REST) Architecture: 6 architectural constraints, client-server decoupling, statelessness, cacheability, uniform interface, and HATEOAS.
...
## 5. REpresentational State Transfer (REST) Architecture
...
### 5.2 The 6 Core Constraints of REST
...
## 6. RESTful HTTP Communications & Protocol Details

### 6.1 HTTP Verbs (Operations on Resources)
...
### 6.2 Headers & Media Content Types (MIME Types)
...
### 6.3 REST URI Path Design Conventions
...
### 6.4 Standard HTTP Response Status Codes
...
3. **REST:** REpresentational State Transfer; a software architectural style that defines constraints for web service communications.
4. **HATEOAS:** Hypermedia as the Engine of Application State; a REST constraint where hypermedia links in responses direct clients to available actions.
5. **Idempotency:** A property of HTTP methods where multiple identical requests leave the server in the same state as a single request.
...
2. List the 6 architectural constraints of REST and define HATEOAS with an example JSON response.
3. Detail the HTTP verbs (GET, POST, PUT, DELETE), their CRUD mappings, idempotency status, and standard return status codes.
4. Analyze the trade-offs of choosing a MERN stack versus a traditional LAMP stack for single page enterprise applications.
